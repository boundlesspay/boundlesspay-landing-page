<template>
  <div class="container mt-5 pt-5">
    <h2 class="text-center pt-5">Contact us</h2>
    <p class="text-center border-bottom pb-5">Get in touch with our team</p>

    <div class="row mt-5 pt-4 border-bottom pb-5">
      <div class="col-md-6">
        <div>
          <h2>Company address</h2>
          <p style="line-height: 40px">403 Oxford Building, Jumeriah Village Circle (JVC), Dubai, United Arab Emirates. <br>

            6th Floor 2 Kingdom Street, London, United Kingdom, W2 6JP</p>

          <div class="d-flex">
            <img src="~assets/img/phone.svg" class="img-fluid" alt="">
            <p class="pt-2 ml-2" >+2349150762004 &nbsp; +447451288551</p>
          </div>
          <div class="d-flex mt-2">
            <img src="~assets/img/email.svg" class="img-fluid" alt="">
            <p class="pt-2 ml-2">info@boundlesspay.com</p>
          </div>
        </div>
      </div>

      <div class="col-md-4 justify-content-end ml-auto">
        <div>
          <img src="~assets/img/location (1).png" class="img-fluid" alt="">
        </div>
      </div>
    </div>

    <div class="row mt-5 pt-5">
      <div class="col-md-6 mt-5">
        <div>
          <h2>Follow us on social media</h2> <br>
          <div class="d-lg-flex mediaa  mt-lg-n3 pb-5">

            <a href="https://twitter.com/boundlesspay" target="https://twitter.com/boundlesspay">
              <img src="~assets/img/vector (16).svg" alt="">
            </a>
            <a href="https://twitter.com/boundlesspay" target="https://twitter.com/boundlesspay">
              <img src="~assets/img/vector (17).svg" alt="">
            </a>
            <a href="https://www.instagram.com/boundlesspay/" target="https://www.instagram.com/boundlesspay/">
              <img src="~assets/img/instagram-black.svg" alt="">
            </a>
            <a href="https://www.facebook.com/boundlesspayofficial/" target="https://www.facebook.com/boundlesspayofficial/">
              <img src="~assets/img/vector (18).svg" alt="">
            </a>

            <a href="https://www.linkedin.com/company/boundlesspay/" target="https://www.linkedin.com/company/boundlesspay/">
              <img src="~assets/img/vector (19).svg" alt="">
            </a>

            <a href="https://www.linkedin.com/company/boundlesspay/" target="https://www.linkedin.com/company/boundlesspay/">
              <img src="~assets/img/vector (20).svg" alt="">
            </a>

            <a href="https://www.linkedin.com/company/boundlesspay/" target="https://www.linkedin.com/company/boundlesspay/">
              <img src="~assets/img/vector (21).svg" alt="">
            </a>

          </div>
        </div>
      </div>

      <div class="col-md-4 justify-content-end ml-auto">
        <div>
          <img src="~assets/img/twitter 1.png" class="img-fluid" alt="">
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "contact-us"
}
</script>

<style scoped>
.mediaa a{
  padding: 0px 10px 0px;
}
</style>
